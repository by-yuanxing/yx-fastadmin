...
class Adminlog extends Backend
{
    ...
    public function _initialize()
    {
        ...
        $groupName = AuthGroup::where('id', 'in', $this->childrenGroupIds)->column('id,name');
		//dump($groupName );
    ...
    public function index()
    {
        ...
        {
			$buildparam = $this->buildparams();
			$buildparams = collection($buildparam)->toArray();
    ...
    public function del($ids = "")
    {
        if ($ids)
        {
			...
			//dump($childrenGroupIds); // 1,2,3,4,5
			
			/* $res = function($query) use($childrenGroupIds) {
                        $query->name('auth_group_access')->field('uid');//uid=1
                    };
			dump($res);	exit; */
			
			//判断条件 1.当前要删除id是否存在 2.当前管理员id是否有权限执行这个删除
