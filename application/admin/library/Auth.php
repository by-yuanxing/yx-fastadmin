...
class Auth extends \fast\Auth
{
    ...
    {
        $admin = Admin::get(['username' => $username]); //模型类可以使用静态调用或者实例化调用两种方式
        if (!$admin) {
        ...
        $admin->save();
        Session::set("admin", $admin->toArray()); //赋值 // 赋值$admin作用域
    ...
    public function autologin()
    {
        ...
        if ($id && $keeptime && $expiretime && $key && $expiretime > time()) {
            ...
            Session::set("admin", $admin->toArray());//赋值给admin模型作用域
            //刷新自动登录的时效
            $this->keeplogin($keeptime);
            return true;
        } else {
    ...
    public function isLogin()
    {
        ...
        $admin = Session::get('admin'); //取值
		//$admin = Session::has('admin'); //判断是否存在
		//dump($admin);
        if (!$admin) {
            return false;
        }
    ...
    public function getGroups($uid = null)
    {
		...
		//echo json_encode(parent::getGroups(1));  //1
    ...
    {
        $uid = is_null($uid) ? $this->id : $uid;
	//	echo json_encode(parent::getRuleList(1));
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        foreach ($params as $k => $v) {
            ...
            if (is_array($v)) { //url的键值为数组时 
                ...
                $color = $colorArr[(is_numeric($nums) ? $nums : strlen($nums)) % $colorNums];//is_numeric() 函数用于检测变量是否为数字或数字字符串。strlen() 函数返回字符串的长度
                $class = 'label';
            }
            //必须nums大于0才显示
            if ($nums) {
		...
		//echo $refererUrl;//来源链接
		
      //  $pinyin = new \Overtrue\Pinyin\Pinyin('Overtrue\Pinyin\MemoryFileDictLoader'); //默认写法
	  vendor('Overtrue.Pinyin.MemoryFileDictLoader');
	  $pinyin = new \Overtrue\Pinyin\Pinyin();
		...
		//条件为不是菜单，字段name含index
        $indexRuleList = \app\admin\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 0)
            ->where('name', 'like', '%/index')
            ->column('name,pid');
			//dump($indexRuleList);exit;
			
        $pidArr = array_filter(array_unique(array_map(function ($item) {return $item['pid'];}, $ruleList)));
		//dump($pidArr);//顶级菜单
        ...
        foreach ($ruleList as $k => &$v) {
            ...
            if (!in_array($v['name'], $userRule)) {
				unset($ruleList[$k]); //unset() 销毁指定的变量。
				continue;
            }
        ...
        $lastArr = array_diff($pidArr, array_filter(array_unique(array_map(function ($item) {return $item['pid'];}, $ruleList))));
		//dump($lastArr);
