...
class Auth
{
    ...
    {
        // 检测用户名或邮箱、手机号是否存在
        if (User::getByUsername($username)) {//动态查询User::getByUsername// 根据Username字段查询用户
            $this->setError('Username already exist'); //setError()设置错误信息
            return false;
        }
        ...
        $params['password'] = $this->getEncryptPassword($password, $params['salt']);//密码加密
        $params = array_merge($params, $extend);

        //账号注册时需要开启事务,避免出现垃圾数据
		Db::startTrans();
        try {
            $user = User::create($params, true);//静态调用create 数组新增

            $this->_user = User::get($user->id); //  $this->_user获取User模型  

            //设置Token
            $this->_token = Random::uuid();
            Token::set($this->_token, $user->id, $this->keeptime); // \app\common\library\token\driver\mysql::set
    ...
    public function login($account, $password)
    {
		...
		//	$field = 'email' ? 'email' : 'mobile' ? 'mobile' : 'username';
		//dump($field);exit;
        $user = User::get([$field => $account]);//// 使用数组查询
